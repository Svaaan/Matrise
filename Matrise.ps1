...
#   .\Matrise.ps1 -Bundle                build a single-file Matrise.bundle.ps1
...
param(
    ...
    [switch]$Unblock,
    [switch]$VerifyAudit,
    # -Bundle writes Matrise.bundle.ps1; add -BundlePath <file> to name it.
    [switch]$Bundle,
    [string]$BundlePath = '',
    [string]$Approve,
    [string]$Deny,
...
. (Join-Path $root 'lib\Devices.ps1')
...
# ---- headless: build a single-file bundle -------------------------------
# Matrise is all PowerShell, so there is no .exe to compile - and an .exe would
# only trip antivirus and application control. This inlines every lib module
# into one .ps1 you can hand over and run directly, no folder to keep together.
if ($Bundle) {
    $libDir = Join-Path $root 'lib'
    $self   = Join-Path $root 'Matrise.ps1'
    if (-not (Test-Path $libDir)) {
        Write-Error "No lib\ folder here. Bundle from the full checkout, not from an already-bundled file."
        exit 1
    }

    $outFile = $(if ($BundlePath) { $BundlePath } else { Join-Path $root 'Matrise.bundle.ps1' })
    $pat = '^\s*\. \(Join-Path \$root ''lib\\(\w+)\.ps1''\)\s*$'

    $out = New-Object System.Collections.Generic.List[string]
    $out.Add('# Matrise - single-file build. GENERATED by: Matrise.ps1 -Bundle')
    $out.Add("# Built $((Get-Date).ToString('yyyy-MM-dd HH:mm')) from $(@(Get-ChildItem $libDir -Filter *.ps1).Count) modules.")
    $out.Add('#')
    $out.Add('# Run it exactly like the folder version - it writes reports\, audit\ and')
    $out.Add('# matrise-layout.json next to itself. Regenerate from the full checkout.')
    $out.Add('')

    $inlined = 0
    foreach ($line in (Get-Content $self)) {
        $m = [regex]::Match($line, $pat)
        if ($m.Success) {
            $name = $m.Groups[1].Value
            $f = Join-Path $libDir "$name.ps1"
            if (-not (Test-Path $f)) { Write-Error "Missing module: $f"; exit 1 }
            $out.Add("# ===================== inlined lib\$name.ps1 =====================")
            foreach ($l in (Get-Content $f)) { $out.Add($l) }
            $out.Add("# ===================== end lib\$name.ps1 =====================")
            $inlined++
        }
        else {
            $out.Add($line)
    ...
    $err = $null
    [void][System.Management.Automation.Language.Parser]::ParseFile($outFile, [ref]$null, [ref]$err)
    $sha = (Get-FileHash $outFile -Algorithm SHA256).Hash

    Write-Output "Bundled $inlined modules into:"
    Write-Output "   $outFile"
    Write-Output "   $([math]::Round((Get-Item $outFile).Length/1KB)) KB   SHA-256 $sha"
    Write-Output ""
    if (@($err).Count -gt 0) {
        Write-Output "WARNING: the bundle did not parse cleanly - $($err[0].Message)"
        exit 1
    }
    Write-Output "Parses clean. Verify it with:  powershell -File `"$outFile`" -List"
    exit 0
}
...
try {
    ...
    . (Join-Path $root 'lib\GuiDevices.ps1')
